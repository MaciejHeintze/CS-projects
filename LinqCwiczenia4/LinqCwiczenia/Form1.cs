...
namespace LinqCwiczenia
{
    public partial class Form1 : Form
    {
        ...
        */
        ...
        {
            var result = Emps.Where(wrt => wrt.Job == "Backend programmer").ToList();
        ...
        {
            var result = Emps.Where(wrt => wrt.Job == "Frontend programmer").Where(wrt1 => wrt1.Salary > 1000).OrderByDescending(i => i.Ename).ToList();
        ...
        private void Przyklad3Button_Click(object sender, EventArgs e)
        {
            ...
            var result = Emps.Max(wrt => wrt.Salary);
        ...
        private void Przyklad4Button_Click(object sender, EventArgs e)
        {
            ...
            var max = Emps.Max(wrt => wrt.Salary);
            var result = Emps.Where(wrt => wrt.Salary == max).ToList();
            ResultsDataGridView.DataSource = result;
        }
        ...
        {
            var result = Emps.Select(wrt => new { Nazwisko = wrt.Ename, Praca = wrt.Job }).ToList();
            ResultsDataGridView.DataSource = result;
        }
        ...
        {
            var result = Emps.GroupBy(wrt => wrt.Job).Select(x => new
            {
                Praca = x.Key,
        ...
        {
            var result = Emps.Where(wrt => wrt.Job == "Backend programmer").Count();
        ...
        {
            var result = Emps.Where(wrt => wrt.Job == "Frontend programmer").OrderByDescending(wrt => wrt.HireDate).Take(1).ToList();
        ...
        private void Przyklad10Button_Click(object sender, EventArgs e)
        {
            ...
            var result = Emps.Select(wrt => new { ename = wrt.Ename, job = wrt.Job, hiredate = wrt.HireDate}).Union(result1).ToList();
            ResultsDataGridView.DataSource = result;
        }
